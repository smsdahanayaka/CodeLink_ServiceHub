...
export default function EditWarrantyCardPage({
  params,
}: {
  ...
  // Fetch warranty card and reference data
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [cardRes, productsRes, customersRes, shopsRes] = await Promise.all([
          fetch(`/api/warranty-cards/${id}`),
          fetch("/api/products?limit=100"),
          fetch("/api/customers?limit=100"),
          fetch("/api/shops?limit=100"),
        ]);

        const [cardData, productsData, customersData, shopsData] = await Promise.all([
          cardRes.json(),
          productsRes.json(),
          customersRes.json(),
          shopsRes.json(),
        ...
        if (cardData.success) {
          ...
          if (productsData.success) {
            const product = productsData.data.find((p: Product) => p.id === card.product.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error?.message || "Failed to update warranty card");
      }
    } catch (error) {
      console.error("Error updating warranty card:", error);
      toast.error("Failed to update warranty card");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Update warranty card details"
        actions={
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Update invoice and purchase information</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
